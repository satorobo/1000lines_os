...
void switch_context(uint32_t *prev_sp, uint32_t *next_sp) {
  __asm__ __volatile__(
    ...
    "sw s10, 11 * 4(sp)\n"
    "sw s11, 12 * 4(sp)\n"
    "sw sp, (a0)\n" // a0 stores *prev_sp -> update *prev_sp
    "lw sp, (a1)\n" // a1 stores *next_sp
    "lw ra,  0  * 4(sp)\n"
    "lw s0,  1  * 4(sp)\n"
    "lw s1,  2  * 4(sp)\n"
...
struct virtio_virtq *virtq_init(unsigned index) {
  ...
  //    `-> Skip
  // 4. Allocate and zero the queue pages in contiguous virtual memory, aligning the Used Ring to an optimal boundary (usually page size).
  //    `-> Skip
...
void virtio_blk_init(void) {
  ...
  //    reading and possibly writing the devices's virtio configuration space, and population of virtqueues.
...
// Notify a new request to the device.
